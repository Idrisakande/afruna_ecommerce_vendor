...
export default function useSearchOrders<T extends TData>({
  data = [],
  period,
}: {
  data: T[];
  ...
  useEffect(() => {
    ...
    const filterData = () => {
      ...
      const filterByStatus = filterBySearchTextResult.filter((item) => {
        if (statusFilter) {
          return item.deliveryStatus.toLowerCase() === statusFilter.toLowerCase();
        }
        return true; // If no status is selected, return all data
      });

      // Apply time period filtering if timePeriod is provided
      let filteredData = filterByStatus;
      if (timePeriod?.toLowerCase()) {
        filteredData = filterDataByTimePeriod(filterByStatus, timePeriod);
      }
