...
import { T_convo } from "@/types/user.type";

export default function useSearchReviews<T extends T_convo>({
	data = [],
	period,
}: {
	data: T[];
...
function filterDataByTimePeriod<T extends T_convo>(
	data: T[],
	timePeriod: string,
) {
...
function sorting<T extends T_convo>(
	items: T[],
